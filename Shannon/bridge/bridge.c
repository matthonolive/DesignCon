/* bridge.c — host mediator. Build via Makefile 'workload' target.
 *   ./bridge gen [out.h] [n_export]
 * Drives the synthetic PAM-M link, trains the RX FFE to a partial-response
 * target, quantises, runs the EXACT fixed-point FFE+MLSE to align ground
 * truth, and emits the generated data header. */
...
#define TAP_Q      14
#define SAMP_Q     15
#define LVL_Q      15          /* symbol levels stored in Q15 (normalised) */
#define TB_DEPTH   16
#define FFE_DELAY  6
...
static int q_round(double x,int qb,int lo,int hi){
    long v=lrint(x*(double)(1<<qb)); if(v<lo)v=lo; if(v>hi)v=hi; return (int)v;
}
...
int bridge_gen(const char *out,int n_export){
    ...
    int *sym=malloc(NSYM*sizeof(int)); double *rx=malloc(NSYM*sizeof(double));
    srand(2024);
    for(int i=0;i<NSYM;i++) sym[i]=rand()%M;
    syn_generate(sym,NSYM,h,hlen,NOISE_SIG,ADC_BITS,rx);

    /* LMS: train FFE so ffe(rx) ~= PR-target conv symbols, delayed */
    double w[RX_FFE_LEN]={0}; w[FFE_DELAY]=1.0; double rb[RX_FFE_LEN]={0};
    double mu=0.01, mse=0; int DELAY=FFE_DELAY+1;
    ...
    int16_t levels[8]; for(int j=0;j<M;j++) levels[j]=(int16_t)q_round(syn_amp(j),LVL_Q,-32768,32767);
    int16_t *stim=malloc(n_export*sizeof(int16_t));
    for(int n=0;n<n_export;n++) stim[n]=(int16_t)q_round(rx[n],SAMP_Q,-32768,32767);

    /* run exact fixed kernel to get decision sequence + slicer for comparison */
    int64_t pm[256]; int8_t tbs[TB_DEPTH*256],tbp[TB_DEPTH*256]; int32_t etab[256*8];
    mlse_t ml; mlse_init(&ml,pm,tbs,tbp,etab,tgt,levels,ns,M,L,TB_DEPTH,LVL_Q);
    int16_t fbuf[RX_FFE_LEN]={0};
    int *dec=malloc(n_export*sizeof(int)), *sdec=malloc(n_export*sizeof(int)), ndec=0;
    for(int n=0;n<n_export;n++){
        int16_t y=ffe_fix(fbuf,taps,stim[n]);
        sdec[n]=syn_slice(y/32768.0);
        int d=mlse_step(&ml,y);
        if(d>=0) dec[ndec++]=d;
    }

    /* align decisions to ground truth */
    ...
    long me=0,mt=0; for(int d=SKIP;d<ndec;d++){int t=d+bestg; if(t<0||t>=NSYM)continue; mt++; me+=(dec[d]!=sym[t]);}
    long se=0,st=0; int sg=0; long sb=n_export+1;
    for(int g=-(RX_FFE_LEN+L+8);g<=8;g++){long mis=0,c=0;
        for(int n=0;n<n_export;n++){int t=n+g; if(t<0||t>=NSYM)continue; mis+=(sdec[n]!=sym[t]);c++;}
        if(c>n_export/2&&mis<sb){sb=mis;sg=g;}}
    for(int n=SKIP;n<n_export;n++){int t=n+sg; if(t<0||t>=NSYM)continue; st++; se+=(sdec[n]!=sym[t]);}

    /* emit header */
    FILE *fp=fopen(out,"w"); if(!fp){perror("open");return 1;}
    fprintf(fp,
      "/* serdes_workload.h — AUTO-GENERATED by bridge (synthetic PAM-%d link).\n"
      " * FFE taps shaped to a PR target, the target + Q15 symbol levels for the\n"
      " * MLSE, the captured RX stream, and aligned ground-truth symbols.\n"
    ...
    printf("bridge gen: wrote %s  (PAM-%d, %d-state trellis)\n",out,M,ns);
    printf("  FFE train MSE=%.3e  lag g=%d  SKIP=%d  decisions=%d\n",mse,bestg,SKIP,ndec);
    printf("  self-check  MLSE   errors = %ld / %ld  (%.2e)\n",me,mt,mt?(double)me/mt:0);
    printf("  self-check  slicer errors = %ld / %ld  (%.2e)  <- MLSE should be <=\n",se,st,st?(double)se/st:0);
