 ...
 *
 * The reset/startup code (_start) lives in start.c, NOT here, so that
 * scheduler.c / tasks.c / main.c can all include this header without
 * emitting duplicate _start symbols.
 *
 * Porting to the Milk-V Duo (CV1800B / T-Head C906): change the addresses
 * and MTIME_HZ below to the C906's CLINT/UART; the rest is portable.
 */
#ifndef BARE_H
#define BARE_H
#include <stdint.h>

/* ── QEMU 'virt' addresses (swap these for the Duo) ──────────────────── */
